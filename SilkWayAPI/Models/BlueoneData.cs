...
namespace SilkwayAPI.Models
{
    ...
    public class BlueoneData
    {
        ...
        private async Task<XDocument> XmlData()
        {
            ...
            string request = @"<x:Envelope xmlns:x=""http://schemas.xmlsoap.org/soap/envelope/"" xmlns:fli=""http://www.blueonesoftware.com/webservices/Flights"">
                ...
                <x:Body>
                    <fli:FlightsRequest>
                        ...
                        <fli:std_start>" + DateTime.UtcNow.AddDays(-1).ToString("s", System.Globalization.CultureInfo.InvariantCulture) + @"</fli:std_start>
                        <fli:std_end>" + DateTime.UtcNow.AddDays(2).ToString("s", System.Globalization.CultureInfo.InvariantCulture) + @"</fli:std_end>
                        <fli:flight>
            ...
            streamWriter.Close();
            //Get the Response    
            var wr = await httpRequest.GetResponseAsync();
            StreamReader srd = new StreamReader(wr.GetResponseStream());
            string resulXmlFromWebService = await srd.ReadToEndAsync();
            return XDocument.Parse(resulXmlFromWebService);
        }

        public FlightsResponse FlightInfo
        {
            get {
                var response = XmlData().Result.Descendants(XmlNs + "FlightsResponse").FirstOrDefault();
